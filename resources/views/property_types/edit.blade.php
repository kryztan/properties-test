        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
            <div class="md:col-span-1">
                <div class="px-4 sm:px-0">
                    <h3 class="text-lg font-medium leading-6 text-gray-900">Edit Property Type</h3>
                </div>
            </div>
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form action="{{ route('property-types.update', ['property_type' => $property_type->id]) }}" method="POST">
                    @csrf
                    @method('PATCH')
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="name" id="name" autocomplete="name"
                                           ...
                                           value="{{ old('name') ?? $property_type->name }}" required>
                                </div>
                                @error("name")
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="description" id="description"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('description') ?? $property_type->description }}" required>
                                </div>
                                @error("description")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    <div class="flex items-start my-2">
                                        <div class="flex items-center h-5">
                                            <input id="is_active" name="is_active" type="checkbox"{{ $property_type->is_active ? ' checked' : '' }}
