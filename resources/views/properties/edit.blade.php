    ...
    <div class="my-10 sm:mt-0">
        ...
        <div class="md:grid md:grid-cols-3 md:gap-6">
            ...
            <div class="mt-5 md:mt-0 md:col-span-2">
                <form action="{{ route('properties.update', ['property' => $property->slug]) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PATCH')
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="description" id="description"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('description') ?? $property->description }}" required>
                                </div>
                                @error("description")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <select id="council_tax_band" name="council_tax_band"
                                        ...
                                        @foreach (range('A', 'H') as $council_tax_band)
                                            <option value="{{ $council_tax_band }}"{{ $property->council_tax_band == $council_tax_band ? ' selected' : '' }}>
                                                {{ $council_tax_band }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <select id="currency" name="currency" data-current-value="{{ $property->currency }}"
                                            class="mt-1 block w-full py-2 px-3 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 sm:text-sm">
                                    </select>
                                </div>
                                @error("currency")
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="tenure" id="tenure"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('tenure') ?? $property->tenure }}" required>
                                </div>
                                @error("tenure")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                    ...
                    <div class="shadow overflow-hidden sm:rounded-md mb-3">
                        <div class="px-4 py-5 bg-white sm:p-6">
                            ...
                            <input id="delete-media-idx" type="hidden" name="delete_media_idx" value="[]">

                            @foreach($images as $index => $image)
                                <div class="media-container flex justify-left items-center">
                                    <img src="{{ $image->getFullUrl() }}" alt="image"
                                         class="p-3 border my-2 max-h-52 shadow-sm border-gray-300 rounded-md">

                                    <button type="button" data-media="{{ $index }}"
                                        class="delete-media bg-red-500 border border-transparent focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 font-medium hover:bg-red-700 ml-5 px-4 py-2 rounded-md shadow-sm text-sm text-white h-fit">
                                        <i class="fas fa-trash"></i>
                                    </button>
                                </div>
                            @endforeach

                            <div id="image-container" data-count="<?= count($images) < 10 ? '1' : '0' ?>">
                            @if(count($images) < 10)
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="address_line_1" id="address_line_1"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('address_line_1') ?? $property->address->address_line_1 }}" required>
                                </div>
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="address_line_2" id="address_line_2"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('address_line_2') ?? $property->address->address_line_2 }}" required>
                                </div>
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="city" id="city"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('city') ?? $property->address->city }}" required>
                                </div>
                                @error("city")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="county" id="county"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('county') ?? $property->address->county }}" required>
                                </div>
                                @error("county")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="postcode" id="postcode"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('postcode') ?? $property->address->postcode }}" required>
                                </div>
                                @error("postcode")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                            ...
                            <div class="grid grid-cols-6 gap-6">
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="latitude" id="latitude"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('latitude') ?? $property->address->latitude }}" required>
                                </div>
                                @error("latitude")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
                                ...
                                <div class="col-span-6 sm:col-span-4">
                                    ...
                                    <input type="text" name="longitude" id="longitude"
                                           class="p-3 border mt-1 focus:ring-blue-500 focus:border-blue-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"
                                           value="{{ old('longitude') ?? $property->address->longitude }}" required>
                                </div>
                                @error("longitude")
                                <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                                @enderror
        ...
        <script>
            $(document).ready(function() {
                ...
                $.get('https://openexchangerates.org/api/currencies.json', function(data) {
                    ...
                    let value = currency.data('currentValue');
                    let content = ``;

                    for (const [currencyCode, currencyName] of Object.entries(data)) {
                        content += `<option value="${currencyCode}"${currencyCode == value ? ' selected' : ''}>${currencyCode}</option>`;
                    }
                ...
                $('.delete-media').click(function () {
                    let input = $('#delete-media-idx');
                    let mediaIndex = $(this).data('media');

                    let mediaArr = JSON.parse(input.val());
                    mediaArr.push(mediaIndex);
                    input.val(JSON.stringify(mediaArr));

                    $(this).closest('.media-container').remove();
                });

                $('#add-image').click(function () {
                    let existingCount = $('.media-container').length;

                    let container = $('#image-container');
                    let count = container.data('count');
                    count++;

                    if (count + existingCount > 10) {
                        return;
                    }
